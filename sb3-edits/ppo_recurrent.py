# This file is modified from https://github.com/Stable-Baselines-Team/stable-baselines3-contrib/blob/master/sb3_contrib/ppo_recurrent/ppo_recurrent.py
...
class RecurrentPPO(OnPolicyAlgorithm):
    ...
    def collect_rollouts(
        ...
        print("STARTING ROLLOUT", n_rollout_steps, "n_rollout_steps")

        lstm_states = deepcopy(self._last_lstm_states)

        if self.use_preamble:

            preamble = self.env.get_attr("preamble")
            ...
            print("PREAMBLESHAPE", np.array(preamble).shape)

            start = time.time()
            with th.no_grad():
                for i in range(preamble.shape[1]):
                    obs_tensor = obs_as_tensor(np.stack([preamble[:, i, :, :]] * 3, axis=1), self.device)
                    self._last_episode_starts = [True] * self.cores if i == 0 else [False] * self.cores
                    episode_starts = th.tensor(self._last_episode_starts, dtype=th.float32, device=self.device)
                    actions, values, log_probs, lstm_states = self.policy.forward(obs_tensor, lstm_states, episode_starts)
                    # print(self._last_episode_starts)
            end = time.time()
            print(f"PREAMBLE INFERENCE TIME: {end - start}")
        ...
        while n_steps < n_rollout_steps:
            ...
            # print(self._last_obs.shape , "SHAPE")
            # cv2.imshow('frame', self._last_obs[1,0,:,:])
            # cv2.waitKey(1)
            with th.no_grad():
